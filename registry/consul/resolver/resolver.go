...
type consulClientConn struct {
	...
	ruscb UpdateStateCallBack
}

// NewConsulClientConn 创建服务发现客户端连接
func NewConsulClientConn(ruscb UpdateStateCallBack) resolver.ClientConn {
	return &consulClientConn{ruscb:ruscb}
}
...
// ResolverUpdateStateCallBack 服务状态更新回调
type UpdateStateCallBack interface {
	Do(state resolver.State)
}

// StartConsulResolverWithState 启动服务发现
func StartConsulResolverWithState(consulAddr string, serviceName string,ruscb UpdateStateCallBack) (schema string, err error) {
	builder := NewConsulBuilder(consulAddr)
	target := resolver.Target{Scheme: builder.Scheme(), Endpoint: serviceName}
	_, err = builder.Build(target, NewConsulClientConn(ruscb), resolver.BuildOption{})
	if err != nil {
...
func StartConsulResolver(consulAddr string, serviceName string) (schema string, err error) {
	return StartConsulResolverWithState(consulAddr,serviceName,nil)
}