...
func main() {
	flag.IntVar(&port,"port",9999,"--port default 9999")
	...
	go func() {
		for i:=0;i< 12 ; i++ {
			tname := fmt.Sprintf("%v%v","number",i)
			//tp,err := cluster.GetPidWithName(tname)
			//if err !=nil {
			//	log.Println(err)
			//}
			resp,err := RPCSendMessageToSomeOne(tname,name,"ni hao a !",pid)
			if err != nil{
				log.Println("resp err : ",err)
				continue
			}
			log.Println("resp",resp.MessageID,resp.ChatMessageStatus,err)
		}
	}()
	cluster.Run()
}
